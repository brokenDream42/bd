-- ============================================================
-- Часть 4: Пакет CRUD-процедур для 3-х выбранных сущностей
--   SELLERS, CUSTOMERS, SALES
-- ============================================================

PROMPT >>> Создание спецификации пакета pkg_entity_crud...

CREATE OR REPLACE PACKAGE pkg_entity_crud IS
    -- SELLERS
    PROCEDURE add_seller(
        p_full_name        IN  VARCHAR2,
        p_retail_outlet_id IN  NUMBER,
        p_salary_rate      IN  NUMBER,
        p_id               OUT NUMBER
    );

    PROCEDURE upd_seller(
        p_id               IN NUMBER,
        p_full_name        IN VARCHAR2,
        p_retail_outlet_id IN NUMBER,
        p_salary_rate      IN NUMBER
    );

    PROCEDURE del_seller(p_id IN NUMBER);

    -- CUSTOMERS
    PROCEDURE add_customer(
        p_full_name       IN  VARCHAR2,
        p_characteristics IN  VARCHAR2,
        p_id              OUT NUMBER
    );

    PROCEDURE upd_customer(
        p_id              IN NUMBER,
        p_full_name       IN VARCHAR2,
        p_characteristics IN VARCHAR2
    );

    PROCEDURE del_customer(p_id IN NUMBER);

    -- SALES
    PROCEDURE add_sale(
        p_product_id       IN  NUMBER,
        p_quantity         IN  NUMBER,
        p_sale_price       IN  NUMBER,
        p_seller_id        IN  NUMBER,
        p_retail_outlet_id IN  NUMBER,
        p_customer_id      IN  NUMBER,
        p_id               OUT NUMBER
    );

    PROCEDURE upd_sale(
        p_id               IN NUMBER,
        p_product_id       IN NUMBER,
        p_quantity         IN NUMBER,
        p_sale_price       IN NUMBER,
        p_seller_id        IN NUMBER,
        p_retail_outlet_id IN NUMBER,
        p_customer_id      IN NUMBER
    );

    PROCEDURE del_sale(p_id IN NUMBER);
END pkg_entity_crud;
/

PROMPT >>> Создание тела пакета pkg_entity_crud...

CREATE OR REPLACE PACKAGE BODY pkg_entity_crud IS

    -- ===================================================
    -- SELLERS
    -- ===================================================
    PROCEDURE add_seller(
        p_full_name        IN  VARCHAR2,
        p_retail_outlet_id IN  NUMBER,
        p_salary_rate      IN  NUMBER,
        p_id               OUT NUMBER
    ) IS
    BEGIN
        INSERT INTO sellers (full_name, retail_outlet_id, salary_rate)
        VALUES (p_full_name, p_retail_outlet_id, p_salary_rate)
        RETURNING id INTO p_id;
    END add_seller;

    PROCEDURE upd_seller(
        p_id               IN NUMBER,
        p_full_name        IN VARCHAR2,
        p_retail_outlet_id IN NUMBER,
        p_salary_rate      IN NUMBER
    ) IS
    BEGIN
        UPDATE sellers
        SET full_name        = p_full_name,
            retail_outlet_id = p_retail_outlet_id,
            salary_rate      = p_salary_rate
        WHERE id = p_id;

        IF SQL%ROWCOUNT = 0 THEN
            RAISE_APPLICATION_ERROR(-20001, 'Продавец с ID=' || p_id || ' не найден');
        END IF;
    END upd_seller;

    PROCEDURE del_seller(p_id IN NUMBER) IS
    BEGIN
        DELETE FROM sellers WHERE id = p_id;
        IF SQL%ROWCOUNT = 0 THEN
            RAISE_APPLICATION_ERROR(-20002, 'Продавец с ID=' || p_id || ' не найден');
        END IF;
    END del_seller;

    -- ===================================================
    -- CUSTOMERS
    -- ===================================================
    PROCEDURE add_customer(
        p_full_name       IN  VARCHAR2,
        p_characteristics IN  VARCHAR2,
        p_id              OUT NUMBER
    ) IS
    BEGIN
        INSERT INTO customers (full_name, characteristics)
        VALUES (p_full_name, p_characteristics)
        RETURNING id INTO p_id;
    END add_customer;

    PROCEDURE upd_customer(
        p_id              IN NUMBER,
        p_full_name       IN VARCHAR2,
        p_characteristics IN VARCHAR2
    ) IS
    BEGIN
        UPDATE customers
        SET full_name       = p_full_name,
            characteristics = p_characteristics
        WHERE id = p_id;

        IF SQL%ROWCOUNT = 0 THEN
            RAISE_APPLICATION_ERROR(-20003, 'Покупатель с ID=' || p_id || ' не найден');
        END IF;
    END upd_customer;

    PROCEDURE del_customer(p_id IN NUMBER) IS
    BEGIN
        DELETE FROM customers WHERE id = p_id;
        IF SQL%ROWCOUNT = 0 THEN
            RAISE_APPLICATION_ERROR(-20004, 'Покупатель с ID=' || p_id || ' не найден');
        END IF;
    END del_customer;

    -- ===================================================
    -- SALES
    -- ===================================================
    PROCEDURE add_sale(
        p_product_id       IN  NUMBER,
        p_quantity         IN  NUMBER,
        p_sale_price       IN  NUMBER,
        p_seller_id        IN  NUMBER,
        p_retail_outlet_id IN  NUMBER,
        p_customer_id      IN  NUMBER,
        p_id               OUT NUMBER
    ) IS
    BEGIN
        INSERT INTO sales (product_id, quantity, sale_price, seller_id, retail_outlet_id, customer_id)
        VALUES (p_product_id, p_quantity, p_sale_price, p_seller_id, p_retail_outlet_id, p_customer_id)
        RETURNING id INTO p_id;
    END add_sale;

    PROCEDURE upd_sale(
        p_id               IN NUMBER,
        p_product_id       IN NUMBER,
        p_quantity         IN NUMBER,
        p_sale_price       IN NUMBER,
        p_seller_id        IN NUMBER,
        p_retail_outlet_id IN NUMBER,
        p_customer_id      IN NUMBER
    ) IS
    BEGIN
        UPDATE sales
        SET product_id       = p_product_id,
            quantity         = p_quantity,
            sale_price       = p_sale_price,
            seller_id        = p_seller_id,
            retail_outlet_id = p_retail_outlet_id,
            customer_id      = p_customer_id
        WHERE id = p_id;

        IF SQL%ROWCOUNT = 0 THEN
            RAISE_APPLICATION_ERROR(-20005, 'Продажа с ID=' || p_id || ' не найдена');
        END IF;
    END upd_sale;

    PROCEDURE del_sale(p_id IN NUMBER) IS
    BEGIN
        DELETE FROM sales WHERE id = p_id;
        IF SQL%ROWCOUNT = 0 THEN
            RAISE_APPLICATION_ERROR(-20006, 'Продажа с ID=' || p_id || ' не найдена');
        END IF;
    END del_sale;

END pkg_entity_crud;
/

PROMPT >>> Пакет pkg_entity_crud создан.
